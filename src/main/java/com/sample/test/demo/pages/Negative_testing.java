...
public class Negative_testing extends TestBase {
	final static Logger logger = Logger.getLogger(Negative_testing.class);
	private String pizza1 = PizzaTypes.MEDIUM_TWOTOPPINGS.getDisplayName();
	private String notoping = PizzaTypes.SMALL_NOTOPPINGS.getDisplayName();
	...
	public Negative_testing pickingpizatype(PropertiesHandler locators) {
		WebElement pik1 = driver.findElement(By.id(locators.reader("pizza1")));
		lib.dropDown(pik1, notoping);
		lib.customWait(2);
		logger.info(
				"Picking pizza with No Topping--> Here: I picking no topping pizza on purrpose, but I will pick two more toppings.");
		return this;
	}

	public Negative_testing pickingtopping1(PropertiesHandler locators) {
	...
	public Negative_testing pickingtopping2(PropertiesHandler locators) {
	...
	public Negative_testing enteringquantity(PropertiesHandler locators) {
	...
	public Negative_testing asserting(PropertiesHandler locators) {
		WebElement cost = driver.findElement(By.id(locators.reader("pizza1Cost")));
		String actualcost = cost.getText();
		SoftAssert soft = new SoftAssert(); // Validating without hard-stop.
		soft.assertEquals(actualcost, pizza1cost);
		return this;
	}

	public Negative_testing enteringname(PropertiesHandler locators) {
	...
	public Negative_testing enteringemail(PropertiesHandler locators) {
	...
	public Negative_testing enteringphone(PropertiesHandler locators) {
	...
	public Negative_testing radiobutnCredit(PropertiesHandler locators) {
	...
	public Negative_testing placeorderbutn(PropertiesHandler locators) {
	...
	public Negative_testing assertingsuccess(PropertiesHandler locators) {
